"""Fan extension of the preserved ALE run loop.

Only the fan momentum RHS, fan diagnostics and provenance are added. The base
run loop is copied intentionally: existing delivered bundles hash solver.py.
Shared pressure, transport, geometry and turbulence operators are imported
unchanged. A hash guard and no-fan parity tests require review if the base moves.
"""
...
import time
import numpy as np
from . import solver as base_solver
from .solver import eddy_viscosity, cross_stress
from .mesh import opening
from .operators import Projector, face_velocity, relative_flux, transport, interpolate
from ..scenarios.geometry import ScenarioMesh
from .fan_forcing import FanArray, FanForcing

BASE_SOLVER_SHA256 = '7fd4f404a77425297a5a9e26bdc5e1b477fb6271cdaf7666f679691ff50ba273'


def run_case(cfg,destination,progress=True,mesh_factory=ScenarioMesh,*,fans):
    if hashlib.sha256(Path(base_solver.__file__).read_bytes()).hexdigest()!=BASE_SOLVER_SHA256:
        raise RuntimeError("Base ALE solver changed; review fan extension and repeat parity tests")
    forcing=FanForcing(cfg,fans)
    fan_props=fans.properties(cfg)
    ...
    (destination/'fan_config.json').write_text(json.dumps(fan_props,indent=2)+'\n')
    ...
    def diagnostics(t,mesh,cc,uu,extra):
        ...
        row['fans_on']=int(fans.active(cfg,t))
        row['fan_force_n']=len(fans.heights_above_floor_m)*fan_props['force_n_per_fan']*row['fans_on']
        row['roi_mean_speed_m_s']=float(np.dot(weights,np.linalg.norm(uu,axis=1))/weights.sum())
        return row
    ...
    while time_s<cfg.duration_s-1e-10:
        ...
        star,_=transport(m,u,ri,rb,g0,g1,dt,nu,bc,source=cross_stress(m,nu,grad)+forcing.source(m,time_s+dt/2),wall_values=wall,reaction=drag)
    ...
    summary=dict(status='EXECUTED_UNQUALIFIED',wall_seconds=time.monotonic()-started,steps=step,cells=m.size,
        ...
        scientific_status='DIAGNOSTIC_ONLY_UNVERIFIED_FAN_PERFORMANCE',
        fan_model=fan_props,base_solver_sha256=BASE_SOLVER_SHA256,
        fan_forcing='Cell-integrated Gaussian momentum source before pressure projection; no scalar source',
