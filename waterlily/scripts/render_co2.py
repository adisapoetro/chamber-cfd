#!/usr/bin/env python3
"""Four white-background views from saved WaterLily + Julia CO₂ fields."""
import argparse
import hashlib
import json
...
class Result:
    ...
    def plan(self, ax, r, z, source=False):
        if source:
            ...
            ax.pcolormesh(self.X,self.Y,np.ma.masked_where(mask,self.C[:,:,z]),cmap=self.cmap,norm=self.norm,shading='nearest')
        ...
        for f in self.summary['fan_configuration']:
            p=np.array(f['position']);d=np.array(f['direction'])
            ax.arrow(*p[:2],*(.5*d[:2]),head_width=.12,color='#D88426',length_includes_head=True)
    ...
    def palm_view(self, ax, r, source=False):
        ...
        if source:
            ...
            rgba=self.cmap(self.norm(self.C[:,:,self.z]));rgba[:,:,3]=np.where(self.volume[:,:,self.z]>0,.65,0.)
            ax.plot_surface(self.X,self.Y,np.full_like(self.X,self.xyz[2][self.z]),facecolors=rgba,
        ...
        ax.set_box_aspect((width,depth,height));ax.view_init(elev=22,azim=-64);ax.grid(False)
        for axis in [ax.xaxis,ax.yaxis,ax.zaxis]: axis.set_pane_color((1,1,1,1))
        ax.set_title('Uptake on representative leaflets' if source else 'Simple palm · saved concentration slice',fontsize=10)
    ...
    def draw(self, view, r):
        ...
        names={'c2_202502_cycle':'C2 · central fans and leaflet uptake','fan_sections':'CO₂ at the three fan levels',
        ...
        if view in ('c2_202502_cycle','fan_sections'):
            cax=fig.add_axes([.925,.42,.012,.36])
            fig.colorbar(plt.cm.ScalarMappable(norm=self.norm,cmap=self.cmap),cax=cax,label='CO₂ (ppm) · fixed viridis')
...
def main(folder):
    ...
    views=['c2_202502_cycle','fan_sections','operating_timeseries','leaf_source_location']
    for view in views:
    ...
    (folder/'rendering.json').write_text(json.dumps({'renderer_sha256':sha(Path(__file__)),
        ...
        'co2_transport_enabled':True,'colour_limits_ppm':[data.norm.vmin,data.norm.vmax],'background':'white'},indent=2)+'\n')
